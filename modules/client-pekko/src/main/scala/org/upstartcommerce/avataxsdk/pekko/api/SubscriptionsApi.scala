...
import org.upstartcommerce.avataxsdk.client.api.SubscriptionsRootApi
import org.upstartcommerce.avataxsdk.client.pekko.{AvataxCollectionCall, AvataxSimpleCall, Stream}
import scala.concurrent.Future

object SubscriptionsRootApiImpl {
  def apply(requester: Requester, security: Option[Authorization], clientHeaders: Option[ClientHeaders])(
      implicit system: ActorSystem,
      materializer: Materializer
  ): SubscriptionsRootApi[Future, Stream] =
    new ApiRoot(requester, security, clientHeaders) with SubscriptionsRootApi[Future, Stream] {
